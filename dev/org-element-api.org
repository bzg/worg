...
properties attached to it.  An exhaustive list is given in section
[[#attributes]].
...
* Setters
  ...
  obtained using these functions, first copy it (e.g., with
  ~copy-sequence~) before modifying it by side effect.  There is no
