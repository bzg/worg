...
context-free[fn:1][fn:2].  Every other syntactical part only exists
...
** Inlinetasks
   ...
   ~org-inlinetask-min-level~ contiguous characters starting at column
   0, followed by a space and the "END" string.
...
** Entities and LaTeX Fragments
   ...
   They are slow to parse, fragile, redundant, imply false positives
   and do not look good in LaTeX output anyway.  Even the LaTeX
   community suggests to use ~\(...\)~ over ~$...$~.  --- ngz
   #+END_QUOTE
