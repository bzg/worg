...
* The Communication Channel
  ...
  process, with their category (~option~, ~persistent~ or ~local~) and
  their value type.
...
** ~:code-refs~
   ...
   Association list between reference name and real labels in source
   code.  It is used to properly resolve links inside source blocks.
...
** ~:footnote-seen-labels~
   ...
   List of already transcoded footnote labels.  It is used to know
   when a reference appears for the first time –
   cf. [[#footnote-first-reference-p][~org-export-footnote-first-reference-p~]].
...
** ~:genealogy~
   ...
   List of current element's parents types.
...
** ~:headline-alist~
   ...
   Alist between headlines raw name and their boundaries.  It is used
   to resolve /fuzzy/ links – cf. [[#resolve-fuzzy-link][~org-export-resolve-fuzzy-link~]].
...
** ~:included-files~
   ...
   List of files, with full path, included in the current buffer,
   through the ~#+include:~ keyword.  It is mainly used to verify that
   no infinite recursive inclusion happens.
...
** ~:inherited-properties~
   ...
   Properties of the headline ancestors of the current element or
   object.  Those from the closest headline have precedence over the
   others.
...
** ~:point-max~
   ...
   Last ending position in the parse tree.
...
** ~:previous-element~
   ...
   Previous element's type at the same level.
...
** ~:previous-object~
   ...
   Previous object type (or ~plain-text~) at the same level.
...
** ~:total-loc~
   ...
   Contains total lines of code accumulated by source blocks with the
   "+n" option so far.
...
** ~org-export-collect-listings~
   ...
   Return a list of all exportable source blocks in parse tree.
...
** ~org-export-collect-tables~
   ...
   Return a list of all exportable tables in parse tree.
...
** ~org-export-get-coderef-format~
   ...
   [[#inline-image-p][~org-export-inline-image-p~]], [[#resolve-fuzzy-link][~org-export-resolve-fuzzy-link~]].
...
** ~org-export-get-headline-number~
   ...
   Return the section number of an headline, as a list.
...
** ~org-export-handle-code~
   ...
   appropriate.  Also update global lines of code count – [[#total-loc][~:total-loc~]]
   property – and code references /alist/ – [[#code-refs][~:code-refs~]] property.

** ~org-export-included-files~

   Recursively transcode a file specified by an include keyword.
...
** ~org-export-resolve-fuzzy-link~
   ...
   attribute – within the parsed tree, and return an appropriate
   unique identifier when found, or nil.

   See also: [[#solidify-link-text][~org-export-solidify-link-text~]],
   [[#get-coderef-format][~org-export-get-coderef-format~]], [[#inline-image-p][~org-export-inline-image-p~]].
...
** ~org-export-table-format-info~
   ...
   Extract formatting information (alignment, column groups and
   presence of a special column) from a raw table and return it as
   a property list.
