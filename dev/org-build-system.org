...
following programs must be avilable in PATH:
...
created that copies the settings from default.mk.

The template includes some comments that are hopefully
self-documenting.  As long as all necessary programs are available,
the customization should in most cases be limited to setting $prefix
correctly.

*** Example 1 — non-standard Emacs location
