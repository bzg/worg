...
*** Installation from a git clone :Update:

/note: current location of org-fireforg to be updated below/

    It is also possible to clone the git repository first, using:
    : git clone http://code.orgmode.org/r/org-fireforg.git
...
** Workaround for the inability to register a protocol in Firefox on the Mac :Update:
   ...
   - get pull.sh from /Note: current location of 'fireforg' repo to be updated/ and run it.
