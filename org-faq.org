...
#<<quickly-set-tag-of-entry>>
   ...
   You can set tags even more quickly by setting one character
   shortcuts for [[http://orgmode.org/manual/Setting-tags.html#Setting-tags][fast tag selection]].
   
   For global settings, put something like the following in your
   =.emacs= file or create the same settings by typing =M-x
   customize-variable RET org-tag-alist=.
