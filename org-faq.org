...
-----------
  ...
  You would normally work on your =local= branch which includes your custom commits; there's no need to
...
#+END_EXAMPLE
     ...
     - edit the file
     - =git add= the file to mark the conflict resolved
     - =git rebase --continue=
...
#+CAPTION: Example conflict markers
#+BEGIN_EXAMPLE
<<<<<<< HEAD:Makefile
EMACS=emacs22
=======
...
#+END_EXAMPLE
     ...
     conflict markers are <<<<<<<<<< , ======= , and >>>>>>>>>>.  Fix the
     Makefile appropriately and delete the conflict markers.  You
     already edited these lines earlier so fixing it should be
     trivial.
