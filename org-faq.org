...
#+INFOJS_OPT: view:content toc:1 path:http://orgmode.org/worg/code/org-info-js/org-info.js tdepth:1 ftoc:t buttons:0 mouse:underline
...
** How can I install Orgmode through Emacs' Package Manager?
   ...
   3. If you get the following error "*Failed to download `Orgmode'
...
# <<Installing from ELPA-tar>>
   ...
   - Additional Note on =org-install.el= :: Functionality of Orgmode's
     =org-install.el= is supplanted by Package Manager's
     =org-autoloads.el=. Since Package Manager autoloads Orgmode for
     you, the following line =(require 'org-install)= in your =.emacs=
     is no longer required and can be safely removed.
     
** Why should I use ELPA-tarballs in preference to snapshot tarballs?
   
   ELPA-tarballs are particularly recommended for newbie users. Much
   of the following grunt work is done by the Package Manager
   infrastrcuture:
   
   1. Download, Compile, Update of =load-path= and auto-load of
      Orgmode.
   2. Update of Info files

   Furthermore, Orgmode's ELPA-tarball is distributed with up-to-date
   User Manuals.
...
still use the git repository to do so.
...
** How do I download fresh .zip and .tar.gz archives of Org?

   You can download [[http://orgmode.org][org-latest.zip]] or [[http://orgmode.org/org-latest.tar.gz][org-latest.tar.gz]] -- these
   archives are udpated every day at midnight.

   Alternatively, you can download a [[http://repo.or.cz/w/org-mode.git/snapshot][tar.gz snapshot from repo.or.cz]].
...
For instructions on how to stay current with org-mode, consult
[[keeping-current-with-Org-mode-development][this FAQ]] or follow the
instructions on [[http://orgmode.org][the official org-mode site]].
...
- *Longer answer*
  ...
  [[http://www.delorie.com/gnu/docs/calc/calc_163.html][found here as long as the section numbering is kept]].
...
4) [[http://code.google.com/p/yasnippet/][Yasnippet]]
...
Org does not currently have special markup for abstracts, but you can
extend the block-level structural and semantic markup in Org with the
contributed package [[file:org-contrib/org-special-blocks.org][org-special-blocks]] (by Chris Gray). To turn it on,
put this in your =.emacs=:
...
Exporting to Latex gives wraps this in a
...
footnotes, embedded images, and so on. (Note, the lists below are by
