...
** Should I use one big org mode file or multiple files?
...
easy hyperlinks]], along with [[http://orgmode.org/manual/Refiling-notes.html#Refiling-notes][easy refiling of notes and todos]], make it
a delight to maintain private "wiki" consisting of multiple files.

In many ways, it doesn't matter how you organize your org files, since
org-mode's agenda commands make it easy to search your notes and
gather together crucial data in a single view.

Moreover, several org-mode settings (see the [[http://orgmode.org/manual/index.html#Top][manual]] for more details)
can be configured both globally and locally:

1. In your .emacs file or via =M-x customize-variable=. (Globally)
2. In individual buffers using option lines at the top of the file.
   (Locally)
3. Within individual subtrees by using property drawers. (Locally)

Here are only a few ideas for organizing org-mode files: 

- A single file all your projects and notes.
...
[[file:org-gtd-etc.org][this page on using GTD with org.]]
