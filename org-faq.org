...
- A Unicorn has healing powers, and many people turn to Org-mode
  because they hurt from stress, disorganization, submission to
  proprietary software and non-portable file formats.

- It will take forever to learn everything there is to know about a
  unicorn.

- A unicorn can loose a fight against the lion and still live to save
...
** Why doesn't C-c a call the agenda? Why don't all org keybindings work?
#<<setting-global-keybindings>>

Org-mode has a few global keybindings that the user must set
explicitly in an =.emacs= file. These include the customary shortcut
for calling the agenda (=C-c a=). If this keybinding does not work,
make sure that the following lines are in your =.emacs= file:
...
and/or do not conflict with other modes.
...
If the variable is nil, then scheduled items will disappear from the
...
** What is the difference between SCHEDULED, DEADLINE, and regular timestamps?
...
2. Items with a DEADLINE timestamp (set with =C-c C-d=) appear on your
   agenda in advance of the when they are do and remain on your agenda
   until they are done. Add a DEADLINE to an item if you want to make
   sure to complete it by a certain date. (The variable
