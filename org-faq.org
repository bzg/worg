...
#<<convert-plain-lists-to-headlines>>
   ...
   Click =C-c -= or =S-<left>/<right>= while on the first line of an
   item. This will cycle through the various bullet styles available
   for lists: =-, +, *, 1., 1)=.
   
   Thus, by typing =C-c -= on one of the plain list items, the
   following:
   ...
   you'll need to remove the leading white space from each line by
   selecting the region and typing something like =C-u - 4 C-x TAB=.
   
   (Note: If your plain list is not indented, the bullet styles
   available via cycling will not include asterisks, since this would
   conflict with the normal key bindings for operating on headlines.)
   
   To convert a headline to a plain list item, simply type =C-c -=
   while the cursor is on the headline.

   You can use query replace to accomplish the same thing, as Bernt
...
You can accomplish the same thing with a keyboard macro, as Bernt
Hansen explains [[http://article.gmane.org/gmane.emacs.orgmode/10148][in this mailing list post]].
