...
** How do I keep current with bleeding edge development?
   ...
   2. Clone the Org mode Git repository.
      ...
      The mirror is particularily useful if are behind a firewall that
      allows only HTTP (can be quite slow):
...
*** TODO Post a link to the .tar.gz snapshot on orgmode.org
