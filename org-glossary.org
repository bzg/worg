...
org-mode. Click on any item for a brief definition, a more detailed
  ...
  - C-c C-x C-s :: Move the subtree to a separate file. The default
     name of the file is =[filename].org_archive=.
       ...
       - Since this is a relatively drastic action, org-mode offers an
         alternate version of the command (=C-u C-c C-x s=) that
...
If you are wondering what a particular org-mode key combindation,
