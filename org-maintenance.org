...
*last three major versions of Emacs*.

For example, if the current major version of Emacs is 28.x, then we
can safely assume that the latest stable version of Org is compatible
with Emacs 28.x, 27.x and 26.x, but not with Emacs 25.x.
