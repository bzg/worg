...
repository, update the Emacs repo with Org latest release and hosts
the [[https://public-inbox.org/README.html][public-inbox]] instance at [[https://list.orgmode.org][list.orgmode.org]].
...
Some file have a dedicated maintainer (see this [[https://list.orgmode.org/87d015if5g.fsf@gnu.org/][call for help]].)
...
** Compatibility with Emacs version

We aim at keeping the latest stable version of Org compatible with at
least Emacs =V=, =V-1= and =V-2= - where =V= is the current stable version of
Emacs.

For example, if the current stable version of Emacs is 28.x, then the
