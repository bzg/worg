...
* Org is regularily tested against the latest Emacs versions

The =main= branch of Org is tested against the three latest stable
versions of GNU Emacs every four hours.  Failures to run all tests
from the main branch are reported to the [[https://lists.sr.ht/~bzg/org-build-failures][org-build-failures]] mailing
list.

The =bugfix= branch of Org is tested against the three latest stable
versions of GNU Emacs every day.  Failures to run all tests from the
=bugfix= branch are reported to the Org mailing list.
