...
* How Worg publishes itself as a website?
  ...
  This is done in two steps: the first step is to pull Worg repository
  on the server (this is cron'ed every 20 minutes, starting at 00:00),
  the second one is to publish the last version of Worg on the website
  (cron'ed every 20 minutes, starting at 00:10).  
