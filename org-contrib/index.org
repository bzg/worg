...
- [[file:org-index.org][/org-index.el/ -- A personal index for org and beyond]] :: Mark and
     find your favorite org-locations and other points of interest
     easily; create and update a lookup table of references and
     links. When searching, frequently used entries appear at the the
     top and entering some keywords narrows down to matching entries
     only; so the right one can be spotted easily. org-index.el is
     included in the /contrib/ directory of Org.
