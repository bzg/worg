...
- /org-index.el/ -- a personal index for org and beyond :: 
  ...
  searching, frequently used entries appear at the the top and entering some
  keywords narrows down to matching entries only; so the right one can be
  spotted easily. org-index.el is included in the /contrib/ directory of Org.
  Written by /Marc Ihm/.
