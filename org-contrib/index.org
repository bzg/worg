...
- /org-index.el/ -- a personal index for org and beyond :: 
  ...
  sorted by usage count, so that the builtin incremental occur presents often
  used entries first.
  References are essentially small numbers (e.g. "R237" or "-455-"), as
  craeated by this package; they are well suited to be used outside of org
  (e.g. within folder names).  Links are normal org-mode links.
