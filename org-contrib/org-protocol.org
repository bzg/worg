...
Then you can invoke the commands from within conkeror with =M-x
org-store-link= and =M-x org-remember=.

Or, if you'd like your familiar emacs keybindings, you can add the
following to your .conkerorrc:

: define_key(default_global_keymap, "C-c r", "org-remember");
: define_key(default_global_keymap, "C-c l", "org-store-link");
