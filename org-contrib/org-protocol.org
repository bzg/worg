...
* About org-protocol.el
  ...
  {{{BeginInfoBox}}}
  @<b> =org-protocol-remember= is now obsoleted by =org-protocol-capture=.@</b>
...
*** Which capture template is used?
    ...
    historical reasons, if a template with the key string "=w=" is defined, this
    one will be choosen by default.  You may change this default by customizing
    the variable =org-capture-default-template=.  This was done to make bookmarks
    used for [[file:./org-annotation-helper.el][org-annotation-helper]] work without changing the template.

    The problem with this solution would be, that only one template can be used
    with the fuction. Luckily, =org-protocol-capture= understands a slightly
...
bookmarklets.  Now in Firefox =ALT-SPC org-capture RET= creates a note
in my Org files.
