...
contrib directory of org-mode.  If it's not loaded or not on your
load-path, you can download it from the git repo and manually load it
with =M-x load-file=.

Next, Org-sync.  It can be fetched from here.  Make sure to checkout
the same revision this tutorial was written with:

#+begin_src sh
git clone git://orgmode.org/org-sync.git
git checkout 2dc5c6a250
#+end_src

Org-sync is still a work in progress so for now you will have to load
every .el in the repo using =M-x load-file=.  Start by =os.el= then
=os-github.el= and =os-bb.el=.

Phew!  Still there?  Good because it's starting to get interesting...

* Tutorial

Let's sync some bugs from [[http://github.com/ostesting/test][this github repo]].
...
the header in =os.el=.  This is the basic template:
