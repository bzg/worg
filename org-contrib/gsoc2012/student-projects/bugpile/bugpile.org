...
characteristics of bugpile were taken:
 ...
 - Emacs centered :: Emacs with its various libraries (Org-mode,
                     Elnode, VC) will be used as the application
                     framework, on top of which the bugtracker is
                     build with Emacs Lisp.
 - Web-Interface :: the bugtracker will have a web interface only. A
                    specialised Emacs mode to interact with the
                    bugtracker is an interesting option for later
                    extensions, but not part of the GSoC 2012 project.
