...
1- Install it.
   - Through the Emacs package system. Add the repository to your =.emacs=:
...
3- Type
   ...
   Beware that the =C-c p= key bindind does not work the first time
   the package is installed through the package system. Later on it
   works properly. Does anyone has a fix ?
