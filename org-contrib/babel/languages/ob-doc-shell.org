...
...or as a standlone script.

#+begin_src sh :results output :shebang :exports both
echo "Defining a number..."
num=14
...
Shell blocks can be tangled, use noweb, accept args, work with stdin,
and more[fn:4].
...
: #+end_src
:
: #+begin_src sh :post multiply_by_2(data=*this*)
:   echo 3
: #+end_src
:
: #+results:
...
: | emacs     |
:
: #+begin_src sh :stdin my-input-block
...
Use the =:shebang= header arg to process a block as a standalone script[fn:10].

Pass a =shebang= (e.g. =#!/bin/bash=) to override the declared shell. If no shebang
is provided, the block language is used (e.g. bash).
