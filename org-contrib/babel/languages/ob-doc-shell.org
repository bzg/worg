...
:
: This is a literate script file.  The script we'll make can be used to ask someone what operating
: system they're using.  First, we define a reference.
:
: #+name: their-os
: Linux
:
: The reference is then passed as stdin into a script which gets executed as a command according to
: the shebang (within a shell specified by the block language).  We get a different response depending
: on who we ask.
:
