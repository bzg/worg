...
We can now include R (and many other programming languages) code in
our documents.  Here's a simple example
...
#+begin_src R :exports code
  ...
  abline(out1)
#+end_src
and appears on p. \pageref{fig:one}.  Note that =x=, =y=, and =out1=
are remembered from the preceding code chunk.  We don't have to
regenerate them.  All code chunks are part of one R "session".
