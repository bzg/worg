...
the optional end specifies a stopping value. If no end is given,
the tics will carry on by the increment value until the max x/y value
...
*** Plot how?
To customize the look of plots, many options are available. Passing these options to gnuplot looks like this:
#+begin_example
...
* Above and Beyond
** Named X-Values/Tics
Sometimes it's desirable to have text as the x-values/xtics instead of
...
While the above example works extremely well when evenly placed tics are
...
call attention to one or more areas on a curve. Perhaps most of a curve
...
- Left Slice
  ...
  - The =ylabel= is set here, and only once. Each slice is
    ...
    multiple labels which is unnecessary since they all share the same label/units.
...
confusing since no divisions give the illusion that the x-axis is
