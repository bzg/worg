...
Note: this list is not updated as frequently as languages are added to
Babel.  The [[http://orgmode.org/w/?p=org-mode.git;a=blob;f=lisp/org.el;hb=HEAD#l142][org-babel-load-languages]] variable holds the up to date
list of all Babel languages available in the Org-mode core (for
copyright reasons some languages are not available as part of the core
and are instead located in the [[http://orgmode.org/w/?p=org-mode.git;a=tree;f=contrib/babel/langs;hb=HEAD][contrib/]] directory).
