...
[fn:1] Languages in the =contrib= directory are not loadable through
       the =org-babel-do-load-languages= variable, but rather must be
       explicitly required (e.g. =(require 'ob-oz)=) after the
       =contrib= directory has been added to your load path.
