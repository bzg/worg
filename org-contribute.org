...
You can also contact some Org maintainers directly [[file:org-maintenance.org::#web-presense-maintainers][on Reddit, GitHub,
IRC, Matrix or other platforms]] if it is easier. However, we prefer the
...
- *Submit patches* to the mailing list.  See how to format [[#first-patch][your first
  ...
  - If you have problems with patch workflow, you may also share a
    link to your public git repository containing changes or submit
...
* If creating a patch is difficult, a simple diff will also do
...
2. Open the Org library you want to modify in the downloaded folder
...
5. =C-x C-w= (=M-x write-file=) in the diff buffer and save it to a
   file like =my-edits.diff=

6. Drop an email to mailto:emacs-orgmode@gnu.org, explaining what you
   did and attaching the "diff" file
