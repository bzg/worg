...
}else{if(m.VIEW==m.SLIDE_VIEW){m.sV(d);m.ss(d);}else{var k=m.VIEW;m.pV(d,1);m.R.DRT=true;
m.R_STATE=OrgNode.SU;m.tG();if(k>m.PLAIN_VIEW){m.tG();}if(k==m.ALL_VIEW){m.tG();}if(f){m.ss(d);
}}}if(0==d||m.INFO_VIEW==m.VIEW){window.scrollTo(0,0);}if(""!=m.OCCUR){m.CI.value=m.OCCUR;
m.RC="o";m.eRC();}if(m.STARTUP_MESSAGE){m.warn("This page uses org-info.js. Press '?' for more information.",true);
...
}if("?/toc/?"!=d){window.location.replace(b.BU+b.Q+b.dT());}if(!b.FIXED_TOC){OrgNode.sE(document.body);
}}else{if(!b.VIEW_BUTTONS){OrgNode.hE(c.BS);}OrgNode.sE(b.N.BS);b.N.sSt(OrgNode.UNFOLDED);
b.N.show();b.N.D.scrollIntoView(true);window.location.replace(b.BU+b.Q+b.dT());}}}},pV:function(d,a){var c=this;
document.onclick=null;document.ondblclick=null;c.VIEW=c.PLAIN_VIEW;OrgNode.hE(c.W);
if(c.IT){OrgNode.hE(c.IT);}OrgNode.sE(c.T);if(c.W.firstChild){c.W.removeChild(c.W.firstChild);
