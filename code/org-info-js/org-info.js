function OrgNode(a,d,c,b,g,f){this.div=a;this.base_id=f;this.idx=-1;this.heading=d;
this.link=c;this.hasHighlight=false;this.parent=g;this.durty=false;this.state=OrgNode.STATE_FOLDED;
...
if(null!=e){e.isOrgNodeFolder=true;this.folder=e;}this.isTargetFor=new Object();this.isTargetFor["#"+this.base_id]=2;
...
this.LOAD_CHECK=window.setTimeout("OrgHtmlManagerLoadCheck()",50);},removeTags:function(a){if(a){while(a.match(this.UNTAG_REGEX)){a=a.substr(0,a.indexOf("<"))+a.substr(a.indexOf(">")+1);
...
var l='javascript:org_html_manager.navigateTo("'+g[1]+'");';this.debug+=g[1]+" + "+g[2]+"\n";
...
};o.onmouseout=function(){org_html_manager.unhighlight_headline(""+h);};}var m="javascript:org_html_manager.navigateTo("+h+")";
if(f>this.NODE.depth){this.NODE=new OrgNode(a,o,m,f,this.NODE,c);}else{if(f==2){this.NODE=new OrgNode(a,o,m,f,this.ROOT,c);
}else{var b=this.NODE;while(b.depth>f){b=b.parent;}this.NODE=new OrgNode(a,o,m,f,b.parent,c);
...
d<n.length;++d){if(!this.TAGS[n[d]]){this.TAGS[n[d]]=new Array();this.SORTED_TAGS.push(n[d]);
}this.TAGS[n[d]].push(h);}}}}this.NODE.hide();return(m);}return(q);},build:function(){var f=this.TITLE.innerHTML;
...
}else{if(b.match(this.EMPTY_START)){b=b.match(this.EMPTY_START)[2];}if(b.length&&b.match(this.EMPTY_END)){b=b.substr(0,b.length-1);
}}if(1==b.length){if("b"==b){this.popHistory();}else{if("B"==b){this.popHistory(true);
...
}return;}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}this.CONSOLE_INPUT.value="";return;},evalReadCommand:function(){var a=this.READ_COMMAND;
var l=this.CONSOLE_INPUT.value;this.endRead();if(""==a||""==l){this.hideConsole();
return;}if(a=="g"){var e=this.SECEX.exec(l);var h=e[1];var f=false;for(var d=0;d<this.SECS.length;
++d){if(this.SECS[d].base_id==h){this.hideConsole();this.navigateTo(this.SECS[d].idx);
return;}}if(!f){this.warn("Goto section: no such section.",false,h);return;}}else{if(a=="s"){if(""==l){return false;
}if(this.SEARCH_HIGHLIGHT_ON){this.removeSearchHighlight();}var j=this.OCCUR;var g=0;
if(l==this.OCCUR){g++;}this.OCCUR=l;this.makeSearchRegexp();for(var d=this.NODE.idx+g;
d<this.SECS.length;++d){if(this.searchTextInOrgNode(d)){this.OCCUR=l;this.hideConsole();
this.navigateTo(this.SECS[d].idx);return;}}this.warn("Search forwards: text not found.",false,this.OCCUR);
this.OCCUR=j;return;}else{if(a=="S"){for(var d=this.NODE.idx+1;d<this.SECS.length;
++d){if(this.searchTextInOrgNode(d)){this.hideConsole();this.navigateTo(this.SECS[d].idx);
return;}}this.warn("Search forwards: text not found.",false,this.OCCUR);return;}else{if(a=="r"){if(""==l){return false;
}if(this.SEARCH_HIGHLIGHT_ON){this.removeSearchHighlight();}var j=this.OCCUR;this.OCCUR=l;
var g=0;if(l==this.OCCUR){g++;}this.makeSearchRegexp();for(var d=this.NODE.idx-g;
d>-1;--d){if(this.searchTextInOrgNode(d)){this.hideConsole();this.navigateTo(this.SECS[d].idx);
return;}}this.warn("Search backwards: text not found.",false,this.OCCUR);this.OCCUR=j;
return;}else{if(a=="R"){for(var d=this.NODE.idx-1;d>-1;--d){l=this.removeTags(this.SECS[d].heading.innerHTML);
if(this.searchTextInOrgNode(d)){this.hideConsole();this.navigateTo(this.SECS[d].idx);
return;}}this.warn("Search backwards: text not found.",false,this.OCCUR);return;}else{if(a=="o"){if(""==l){return false;
}if(this.SEARCH_HIGHLIGHT_ON){this.removeSearchHighlight();}var j=this.OCCUR;this.OCCUR=l;
this.makeSearchRegexp();var b=new Array();for(var d=0;d<this.SECS.length;++d){if(this.searchTextInOrgNode(d)){b.push(d);
}}if(0==b.length){this.warn("Occur: text not found.",false,this.OCCUR);this.OCCUR=j;
return;}this.hideConsole();if(this.PLAIN_VIEW!=this.VIEW){this.plainView();}this.ROOT.durty=true;
this.toggleGlobaly();for(var d=0;d<this.SECS.length;++d){OrgNode.showElement(this.SECS[d].div);
OrgNode.hideElement(this.SECS[d].folder);}for(var d=(b.length-1);d>=1;--d){OrgNode.showElement(this.SECS[b[d]].folder);
}this.showSection(b[0]);}else{if(a==this.READ_COMMAND_ORG_LINK){var k=l.charAt(0);
if("s"==k){this.startRead(this.READ_COMMAND_NULL,"Org-link to this section:","[["+this.BASE_URL+this.getDefaultTarget()+"]["+document.title+", Sec. '"+this.removeTags(this.NODE.heading.innerHTML)+"']]","C-c to copy, ");
window.setTimeout(function(){org_html_manager.CONSOLE_INPUT.select();},100);}else{if("o"==k){this.startRead(this.READ_COMMAND_NULL,"Org-link, occurences of <i>&quot;"+this.OCCUR+"&quot;</i>:","[["+this.BASE_URL+"?OCCUR="+this.OCCUR+"]["+document.title+", occurences of '"+this.OCCUR+"']]","C-c to copy, ");
window.setTimeout(function(){org_html_manager.CONSOLE_INPUT.select();},100);}else{this.warn(k+": No such link type!");
}}}else{if(a==this.READ_COMMAND_HTML_LINK){var k=l.charAt(0);if("s"==k){this.startRead(this.READ_COMMAND_NULL,"HTML-link to this section:",'<a href="'+this.BASE_URL+this.getDefaultTarget()+'">'+document.title+", Sec. '"+this.removeTags(this.NODE.heading.innerHTML)+"'</a>","C-c to copy, ");
window.setTimeout(function(){org_html_manager.CONSOLE_INPUT.select();},100);}else{if("o"==k){this.startRead(this.READ_COMMAND_NULL,"HTML-link, occurences of <i>&quot;"+this.OCCUR+"&quot;</i>:",'<a href="'+this.BASE_URL+"?OCCUR="+this.OCCUR+'">'+document.title+", occurences of '"+this.OCCUR+"'</a>","C-c to copy, ");
window.setTimeout(function(){org_html_manager.CONSOLE_INPUT.select();},100);}else{this.warn(k+": No such link type!");
}}}}}}}}}}},getDefaultTarget:function(b){if(null==b){b=this.NODE;}var c="#"+this.NODE.base_id;
for(var a in b.isTargetFor){if(1==b.isTargetFor[a]){c=a;break;}}return c;},makeSearchRegexp:function(){var a=this.OCCUR.replace(/>/g,"&gt;").replace(/</g,"&lt;").replace(/=/g,"\\=").replace(/\\/g,"\\\\").replace(/\?/g,"\\?").replace(/\*/g,"\\*").replace(/\+/g,"\\+").replace(/\"/g,"&quot;");
