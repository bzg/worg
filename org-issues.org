...
* New Issues [289/307]                                                                             :new:
** NEW Unintended behavior? Links without description
  [2011-01-09 So]
:PROPERTIES:
...
** WAITING (Bug) Abstract block prematurely ended by asterisk
:LOGBOOK:
...
** WAITING Disputed keys (meta left), (meta right)
:LOGBOOK:
...
** WISH Bug: Jumping to a date in the agenda changes view back to 'day' (7.4 (release_7.4.80.g0e5e5))
:LOGBOOK:
...
** INCONSISTENCY Re: (Orgmode) org-add-link-type
:LOGBOOK:
...
** BUG empty lines in datetree capture templates
:LOGBOOK:
