...
** NEW (Babel) (Don't) recenter buffer after evaluating code block
  [2010-12-19 So]
:PROPERTIES:
...
** NEW Very strange indenting behaviour with CLOCKING drawers.
  [2010-12-19 So]
:PROPERTIES:
...
** NEW bug in the agenda bulk actions for rescheduling several items
  [2011-01-02 So]
:PROPERTIES:
...
** NEW Bug in the :VISIBILITY: handling of "folded" PROPERTY?
  [2011-01-02 So]
:PROPERTIES:
...
** NEW (BUG) Table field clipping doesn't handle double-width characters properly
  [2011-01-02 So]
:PROPERTIES:
...
** NEW (BUG) org-cdlatex after environment
  [2011-01-02 So]
:PROPERTIES:
...
** NEW Bug: Items with priority not refile targets? (7.4)
:LOGBOOK:
...
** QUESTION How to use capture to start relative timer? And some other capture issues...
:LOGBOOK:
...
** QUESTION Total time displayed in the modeline?
:LOGBOOK:
...
** QUESTION (babel) creating simple vectors in R
:LOGBOOK:
...
** QUESTION Bug: org lets me mess things up too easily (7.01trans)
:LOGBOOK:
...
** QUESTION Adding tags, grouping tags
:LOGBOOK:
...
** QUESTION (BUG) MobileOrg blocked tasks showing up on agendas                             :Mobile:
:LOGBOOK:
...
*** BUG Bug: Agenda's `Goto Today' doesn't in Day view (7.4)
:LOGBOOK:
...
*** BUG Bug: capture with PROPERTIES (7.3 commit-972b0a58...)
:LOGBOOK:
