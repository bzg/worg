...
** WISH (O) Macro expansion in included files                                                                                                 :Patch:
:LOGBOOK:
...
** NEW (O) org-crypt.el security problem (From: Milan Zamazal)
  [2011-03-20 So]
:PROPERTIES:
...
** NEW (O) (Bug) Verbatim code gets interpreted
  [2011-03-20 So]
:PROPERTIES:
...
** NEW (O) regexp link on windows problem
  [2011-03-20 So]
:PROPERTIES:
...
** NEW (O) (Use ?) Capture and Refile behavior
  [2011-03-20 So]
:PROPERTIES:
...
** NEW (O) goto does not accept remote references for local targets
  [2011-03-20 So]
:PROPERTIES:
...
** NEW (O) Problem opening links that span more than one line
  [2011-03-20 So]
:PROPERTIES:
...
** WISH (O) (PATCH) Make tangling work in an indirect buffer                                                                            :Babel:Patch:
:LOGBOOK:
...
** WAITING (O) "org-nil" error in LaTeX export
:LOGBOOK:
...
** NEW (O) Bug: Minor Bug: COOKIE_DATA property breaks parent statistics (7.5 (release_7.5.24.g5f0ef))
  [2011-03-20 So]
:PROPERTIES:
...
** WAITING (O) Symbol's function definition is void: fill-forward-paragraph
:LOGBOOK:
...
** WISH (O) (dev) org-indent-mode patch
:LOGBOOK:
...
** NEW (O) Bug: Bug in editing table.el tables (7.5 (release_7.5.36.g4e24))
  [2011-06-06 Mo]
:PROPERTIES:
...
** NEW (O) (babel) "Marker does not point anywhere" error?                                                                                    :Babel:
  [2011-06-06 Mo]
:PROPERTIES:
...
** NEW (O) Org-capture does not work with "long" extracts of text
  [2011-06-06 Mo]
:PROPERTIES:
...
** NEW (O) No property change from ‘org-clock-sum’                                                                                          :contrib:
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) (bug) project publish :auto-postamble and :postamble broken for html
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) (BUG) Changing TODO states sometimes modifies the scheduling of the next heading
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW Re: (O) org-add-note not working with winner-mode                                                                                    :emacs24:
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) New contribution: Bill-of-materials --  org-bom.el
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW Re: (O) org-agenda-log-mode doesn't list past scheduled items if org-agenda-skip-scheduled-if-done is t
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) org-kill-line sometimes crashes emacs
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) italics inside quotation marks -> LaTeX not working
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) HTML Postamble is inside Content DIV
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) Bug in latex export of <<links>>
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) syntax bugs in footnotes
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) Feature request: modify italic regexp list to include non-breaking space and other characters
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) HTML export > Resizing an activated inline image
  [2011-06-13 Mo]
:PROPERTIES:
...
** NEW (O) Bug: EOL needs to be converted to Unix for MobileOrg files (7.5 (release_7.5.147.g9ddc))                                          :Mobile:
  [2011-06-26 So]
:PROPERTIES:
...
** NEW (O) Bug: Some bulk operations are slow (7.5 (release_7.5.135.g7021f))
  [2011-06-26 So]
:PROPERTIES:
...
*** DECLINED (ANN) List improvement v.2
