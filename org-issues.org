...
** NEW (BUG) org-babel :session breaks returning things with python?
  [2010-11-14 So]
:PROPERTIES:
...
** BUG Bug report: syntax highlighting fails with org-indent-mode and emacs 23.2 
:LOGBOOK:
...
** BUG Executing sh code within sessions
:LOGBOOK:
- State "BUG"        from "NEW"        [2010-11-14 So 19:19]
:END:
  [2010-11-14 So]
...
** BUG Error when running org-babel-execute-buffer -- Wrong type argument: consp, nil       :Babel:
:LOGBOOK:
...
** NEW (PATCH) Quarters added to clocktables                                                                    :Patch:
  [2010-11-28 So]
:PROPERTIES:
...
** INCONSISTENCY Bug: Initial C-c ' invocation just starts haskell-mode "normally" (7.3)
:LOGBOOK:
