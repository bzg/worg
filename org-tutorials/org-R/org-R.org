...
* Using org-R
  ...
  ignore that for now. To make the action happen, use M-x org-R-apply
  with point in the #+R: line. It would make sense to bind it to some
  key. There's also org-R-apply-in-subtree and org-R-apply-in-buffer,
  which visit each org-R block that they find in the current
  subtree/buffer, calling org-R-apply in that block (suggestion from
  Tom Short). So, first example.
