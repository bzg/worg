...
* Table references
   ...
   1. by providing a reference to an org table with
      intable:name-or-id. name-or-id may be either an org-id entry
      identifier, or the #+TBLNAME name of a table. In the latter
      case, infile:/path/to/file.org can optionally be used to specify
      the file containing the table; otherwise the table is looked for
...
#+TBLRR: plot(hclust(dist(x, method="binary")))
...
#+TBLRR: plot(hclust(dist(t(x), method="binary")))
#+TBLR: infile:"org-variables-incidence.org" intable:"incidence-matrix" rownames:t
...
[[file:/usr/local/src/org-etc/Worg/org-tutorials/org-R/data/45/f39291-3abc-4d5b-96c9-3a32f77877a5/org-R-output-652pRO.png][png output]]
