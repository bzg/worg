...
this implementation.    
* Using the tabularx and booktabs packages
...
The following steps assume that =org-babel-load-languages= has an entry =(latex
. t)= and that functions in the Library of Babel are available.  I
have this in =.emacs= (but don't know if it is necessary):
...
:   <<booktabs(table=test-table,align="lX",env="tabularx",width="\\textwidth")>>  
:   \end{table}
:  #+end_src
