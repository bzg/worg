...
The Org-mode LaTeX exporter uses several packages to support special characters
used by =org-entities=.  One of these, =amsmath=, conflicts with
several [[font-section][LaTeX fonts]].  If you want finer control over which packages
are loaded, then it makes sense to define an export class like this
in =.emacs=:
...
peculiar to arbitrary class or style files.  Fortunately, there are
facilities for inline and block-level markup.
