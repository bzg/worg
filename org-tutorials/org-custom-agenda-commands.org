...
containing only appointments---that is, no tasks with schedules and
deadlines:
...
And you can use the following custom command to see appointments
deadlines due within the next 60 days---that is, no scheduled tasks:
