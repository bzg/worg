...
...with the cursor conveniently after the plus sign. He completes 
the query to find inactive timestamps from November...
...
property contains a value, keyword searches simply scan a the whole
buffer for matches and then return the appropriate headlines.
...
nothing of outline tree inheritance. Thus, if Mr. Gnu interested in
...
=BIB_PAGES= property that contain at least for digits (i.e., > 1000
pages).
