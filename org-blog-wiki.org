...
- [[https://ox-hugo.scripter.co][ox-hugo]] :: An Org exporter backed that exports to
     ...
     supports two common Org blogging flows:
  - Exporting multiple Org sub-trees from a single file to multiple
    ...
    such example). It also leverages Org tag and property inheritance,
    and much more!
  - Exporting a single Org file to a single Hugo post.

     See the package's website for complete documentation, and
    hundreds of test cases/feature demonstrations.
