...
* How Worg publishes itself as HTML?
  ...
  Worg into =HTML=. This is done in two steps: the first step is to pull
  the Worg directly on the server (cron'ed every 20 minutes, starting at
  00:00), the second one is to publish the last version of Worg on the
  website (cron'ed every 20 minutes, starting at 00:10).

  So for example, if you push a change in Worg at 00:37, this will be
  pulled on the server at 00:40 and will appear on the website at 00:50.

  The output of both cron jobs are here: 
