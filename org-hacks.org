...
-- Note added 2011-11-12 Sat

Now that org allows ranges on the LHS of table formulas, the multiple formulas
can be condensed down to a single formula:

: #+TBLFM: @1$1..@>$> = remote(FOO, @$#$@#)

Note however that this is bound to be slow for large tables. See the discussion
in the [[http://thread.gmane.org/gmane.emacs.orgmode/22610/focus=23662][mailing list]].
