...
The setting above archives subtrees to files names "2009.org_archive",
"2010.org_archive", and so on. Each file is organized by file name:
...
#+begin_src emacs-lisp
  (defun my-org-reconstitute-file-from-archives ()
  ...
  buffer named *archive-[name-of-original-file].org*."
    ...
    (let* ((arch-files (file-expand-wildcards "~/org/20*.org_archive"))
           ...
           (location-table (cdr (org-get-refile-targets)))
           (comp-list (mapcar 
                       (lambda (x)
                         (nth 1 (split-string (car x) "/")))
                       location-table))
           (name (completing-read "File name: " (delq nil comp-list) nil nil))
           (buf (concat "*archive-" name "*"))
           p temp-file)
      (save-window-excursion
        ...
        (mapc 
         (lambda (x)
           (string-match "^\\([0-9]+\\)\\.org_archive" (file-name-nondirectory x))
