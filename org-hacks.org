...
*** Headings Without Leading Stars (hidestarsfile)
    :PROPERTIES:
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my-org-fileconv-init ()
    ...
    ;; will be lost at the moment when the major mode is changed
      ...
      (when (and re-list (not my-org-fileconv-level-org-p))
        ...
        (let* ((common-re (nth 0 re-list))
          ...
          (while (re-search-forward file-re nil t)
            (goto-char (match-beginning 0))
            ;; faster than lisp call of ins and del for each single char
          ...
          (when (not mod-p) (not-modified))
          ;; avoid jump to header with level > 1 during undo when file
          ;; is saved before undo
