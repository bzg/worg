...
#+index: Compilation!without make
  ...
  Presumably starting with Org 7.9 (and in Git on current master
  already) you'll find functions for creating the autoload files and
  do byte-compilation in =UTILITIES/org-fixup.el=.  To make just the
  autoloads file do:
  ...
  first if =org-version.el= is not available:
  : emacs -batch -Q -L lisp -l ../UTILITIES/org-fixup \
  : --eval '(org-make-org-version "7.8.11" "7.8.11-fake" "/path/to/odt/styles")'

  For older org versions, use the definitions below.  To use this
  function, adjust the variables =my/org-lisp-directory= and
