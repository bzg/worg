...
*** Hex computation
As with [[#time-computation][Times computation]], the following code allows Computation with
...
** Exporting org files
*** Use placement option to specify table fontsize
...
LaTeX export. One can take advantage of this to pass a fontsize
different from the rest of the document by putting something like
=\footnotesize= right after the placement options.

For example during LaTeX export using the ~#+ATTR_LaTeX:~ line below,
exports the associated floating environment as shown in the block
after that.
