...
* Set org-agenda-files by filetag
  - Matt Lundin
...
#+begin_src emacs-lisp
(defun my-org-agenda-files-by-filetag (&optional tag)
  "Limit org agenda files only to those containing filetag."
  (interactive)
  (let* ((tagslist (my-org-get-all-filetags))
	 (ftag (or tag 
		   (completing-read "Tag: " 
				    (mapcar 'car tagslist)))))
    (setq org-agenda-files (cdr (assoc ftag tagslist)))))

(defun my-org-get-all-filetags ()
  "Get list of filetags from all default org-files."
  (let ((files 
	 (my-org-list-files 
	  my-org-agenda-directories my-org-agenda-extensions))
	(tagslist nil)
	x)
...
Calling my-org-agenda-files-by-filetag results in a prompt with all
filetags in your "normal" agenda files. When you select a tag,
org-agenda-files will be set only to those files containing that
filetag.

This functionality relies on a few other functions that abstract the
process of setting org-agenda-files:
...
#+begin_src emacs-lisp
  ...
  (setq my-org-agenda-directories '("~/org/"))
...
ending in ".org" in the directory "~/org/". You can change these values
by setting the variables my-org-agenda-extensions and
my-org-agenda-directories. The function my-org-agenda-files-by-filetag
uses these two variables to determine which files to search for
filetags (i.e., the larger set from which the subset will be drawn).
...
** Org-mime
*note*: [[file:org-contrib/org-mime.org][Org-mime]] has recently been added to the contrib directory of
 Org-mode implementing this functionality for both Wanderlust and
 Gnus.

** non Org-mime approach
