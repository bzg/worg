...
#+index: Compilation!without make
  ...
  Starting with Org 7.9 (and in Git on current master already) you'll
  find functions for creating the autoload files and do
  byte-compilation in =UTILITIES/org-fixup.el=.  When you execute the
  commands below, your current directory must be where org has been
  unpacked into, in other words the file =default.mk= should be found
  in your current directory.  The command =emacs= should be found in
  your =PATH= and start the Emacs version you are using.  To make just
  the autoloads file do:
  ...
  can also edit the version strings there).
  ...
  has quite different quoting rules, so your other option is to start
  Emacs like this
...
#+END_SRC
  ...
  C-e= to evaluate the form.

  For *older* org versions only, you can use the definitions below.
  To use this function, adjust the variables =my/org-lisp-directory=
  and =my/org-compile-sources= to suit your needs.  If you have
